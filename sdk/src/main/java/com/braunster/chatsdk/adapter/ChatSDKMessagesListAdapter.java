...
import com.braunster.chatsdk.activities.util.ViewVideoActivity;
import com.braunster.chatsdk.dao.BMessage;
import com.braunster.chatsdk.dao.BUser;
import com.braunster.chatsdk.dao.core.DaoCore;
import com.braunster.chatsdk.dao.entities.BMessageEntity;
import com.braunster.chatsdk.network.BDefines;
import com.braunster.chatsdk.view.ChatBubbleAudioView;
import com.braunster.chatsdk.view.ChatBubbleImageView;
import com.braunster.chatsdk.view.ChatBubbleVideoView;
...
public class ChatSDKMessagesListAdapter extends BaseAdapter{
    ...
    /* Row types
     * 
     * User type should always be even number and friend an odd one
    ...
    private static final int TYPE_AUDIO_USER = 6;
    private static final int TYPE_AUDIO_FRIEND = 7;
    private static final int TYPE_VIDEO_USER = 8;
    private static final int TYPE_VIDEO_FRIEND = 9;
    ...
    private int textUserRowResId = R.layout.chat_sdk_row_text_message_user, textFriendRowResId = R.layout.chat_sdk_row_text_message_friend,
            ...
            audioUserResId = R.layout.chat_sdk_row_audio_message_user, audioFriendRowResId = R.layout.chat_sdk_row_audio_message_friend,
            videoUserResId = R.layout.chat_sdk_row_video_message_user, videoFriendRowResId = R.layout.chat_sdk_row_video_message_friend;
    ...
    public long getItemId(int i) {
        if(i > listData.size()) return listData.get(i - 1).id;
...
//        if (DEBUG) Log.d(TAG, "Message, Type: " + message.type  + " Row Type: " + type + ", Text: " + message.text);
    ...
    protected View inflateRow(ViewHolder holder){
        ...
        switch (type)
        {
            ...
            case TYPE_AUDIO_USER:
                row = inflater.inflate(audioUserResId, null);

                holder.progressBar = (ProgressBar) row.findViewById(R.id.chat_sdk_progress_bar);
                holder.audio = (ChatBubbleAudioView) row.findViewById(R.id.chat_sdk_audio);

                break;

            case TYPE_AUDIO_FRIEND:
                row = inflater.inflate(audioFriendRowResId, null);

                holder.progressBar = (ProgressBar) row.findViewById(R.id.chat_sdk_progress_bar);
                holder.audio = (ChatBubbleAudioView) row.findViewById(R.id.chat_sdk_audio);

                break;
            case TYPE_VIDEO_USER:
                row = inflater.inflate(videoUserResId, null);

                holder.progressBar = (ProgressBar) row.findViewById(R.id.chat_sdk_progress_bar);
                holder.video = (ChatBubbleVideoView) row.findViewById(R.id.chat_sdk_video);

                break;

            case TYPE_VIDEO_FRIEND:
                row = inflater.inflate(videoFriendRowResId, null);
    ...
    protected void loadMessageData(ViewHolder holder, MessageListItem message){
        switch (type)
        {
            ...
            case TYPE_AUDIO_FRIEND:
                ...
                getBubbleAudioViewFromRow(holder.audio, holder.progressBar,  message);
            ...
            case TYPE_VIDEO_FRIEND:
                ...
                getBubbleVideoViewFromRow(holder.video, holder.progressBar,  message);

                holder.video.setOnClickListener(new showVideoDialogClickListener(message));
    ...
    /** Get a ready image view for row position. The picture will be loaded to the bubble image view in the background using Volley. */
    private ChatBubbleAudioView getBubbleAudioViewFromRow(final ChatBubbleAudioView audio, final ProgressBar progressBar, final MessageListItem message){
        if (DEBUG) Timber.v("getBubbleAudioViewFromRow");

        // Save the message text to the audio tag so it could be found on the onClick.
        audio.setTag(message.text);

        // Loading the url.
        final ChatBubbleAudioView.LoadDone loadDone = new ChatBubbleAudioView.LoadDone() {
            @Override
            public void onDone() {
                if (progressBar.getVisibility() == View.VISIBLE) {
                    progressBar.setVisibility(View.INVISIBLE);
        ...
        if(DEBUG) Timber.v("getBubbleAudioViewFromRow, duration: " + Integer.toString(message.duration));
        ...
        if (StringUtils.isNotEmpty(message.resourcePath) && new File(message.resourcePath).exists())
        {
            ...
            audio.loadFromPath(message.resourcePath, loadDone);
        }
        else
        {
            // Saving the url so we could remove it later on.
            ...
            if(DEBUG) Timber.v("load from url: " + message.url);

            audio.loadFromUrl(message.url, loadDone);
        }
    ...
    /** Get a ready image view for row position. The picture will be loaded to the bubble image view in the background using Volley. */
    private ChatBubbleVideoView getBubbleVideoViewFromRow(final ChatBubbleVideoView video, final ProgressBar progressBar, final MessageListItem message){
        if (DEBUG) Timber.v("getBubbleImageViewFromRow");

        // Save the message text to the video tag so it could be found on the onClick.
        video.setTag(message.text);

        // Loading the url.
        final ChatBubbleVideoView.LoadDone loadDone = new ChatBubbleVideoView.LoadDone() {
            ...
            public void immediate(boolean immediate) {
                if (immediate){
                    ...
                    animateContent(video, null, message.delivered != BMessage.Delivered.No);
                }
                else
                {
                    video.clearCanvas();
        ...
        if (message.dimensions != null)
        {
            if(DEBUG) Timber.v("message dimension != null");
            // Getting the dimensions of the video so we can calc it final size and prepare room for it in the list view.
            RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) video.getLayoutParams();
            params.width = (int) (message.dimensions[0] + video.getImagePadding() + video.getTipSize());
            params.height = message.dimensions[1] + video.getImagePadding();
            ...
            if (StringUtils.isNotEmpty(message.resourcePath) && new File(message.resourcePath).exists())
            {
                // Saving the url so we could remove it later on.
                if (saveCacheKeys)
                    saveCacheKey(message.resourcePath);
                if(DEBUG) Timber.v("file exists");

                String thumbnailUrl = "";
                String [] urls = message.text.split(BDefines.DIVIDER);

                if(urls.length > 1) thumbnailUrl = urls[1];
                if(DEBUG) Timber.v("thumbnail url: " + thumbnailUrl);

                video.loadFromUrl(thumbnailUrl, loadDone, message.dimensions[0], message.dimensions[1]);
            }
            else
            {
                // Saving the url so we could remove it later on.
                if (saveCacheKeys)
                    saveCacheKey(message.url);
                if(DEBUG) Timber.v("else" + message.url);
    ...
    /**
     *  Click listener for a video view
     *  */
    public class showVideoDialogClickListener implements View.OnClickListener{

        private MessageListItem message;

        public showVideoDialogClickListener(MessageListItem message) {
            this.message = message;
        }
        ...
        public void onClick(View v) {
            ...
            if (StringUtils.isNotBlank(message.resourcePath))
            {
                intent.putExtra("loadType", "path");
                intent.putExtra("url", message.resourcePath);
                mActivity.startActivity(intent);
            }
            else if (StringUtils.isNotBlank(message.text))
            {
                String videoUrl = message.text.split(BDefines.DIVIDER)[0];
                // Saving the url so we could remove it later on.
                saveCacheKey(videoUrl);

                if (!BDefines.Options.SaveVideoToDir) {
                    // TODO: save video to dir
                }

                intent.putExtra("loadType", "url");
                intent.putExtra("url", videoUrl);
                mActivity.startActivity(intent);
            }
            else
            {
                chatSDKUiHelper.showAlertToast(mActivity.getString(R.string.message_adapter_load_video_fail));
            }
        }
    }
    ...
    public static class MessageListItem{
        ...
        private MessageListItem(long id, String entityId, int type, int rowType, int status,
                                ...
                                String text, String color, long timeInMillis, int delivered, String resourcePath, String dimensionsString, int duration) {
            this.type = type;
            this.id = id;
            ...
            this.url = getUrl(text, type);

            if(text == null) this.text = url;
        }
        ...
        public static MessageListItem fromBMessage(BMessage message, Long userID, int maxWidth, SimpleDateFormat simpleDateFormat){
            ...
            if(message.getDuration() == null) {
                if(DEBUG) Timber.v("fromBMessage, duration is null");
                message.setDuration(-1);
            }

            Timber.v(Integer.toString(maxWidth));
        ...
        public static List<MessageListItem> makeList(Activity activity, Long userID, List<BMessage> messages, SimpleDateFormat simpleDateFormat){
            ...
            for (BMessage message : messages)
            {
                ...
                if (i.type != BMessage.Type.TEXT && i.type != BMessage.Type.AUDIO && i.dimensions == null)
                {
        ...
        private static int getRowType(int messageType, long senderId, long curUserID){
            ...
            switch (messageType)
            {
                ...
                case BMessage.Type.AUDIO:
                    if (senderId == curUserID)
                        type = TYPE_AUDIO_USER;
                    else type = TYPE_AUDIO_FRIEND;
                    break;

                case BMessage.Type.VIDEO:
                    if (senderId == curUserID)
                        type = TYPE_VIDEO_USER;
                    else type = TYPE_VIDEO_FRIEND;
                    break;
        ...
        private static String getUrl(String text, int type){
            ...
            if (type == BMessageEntity.Type.TEXT)
            {
                url = text;
            }
            else if (type == BMessageEntity.Type.IMAGE)
            {
                if (urls.length > 1)
                {
                    url = urls[1];
...
//                e.printStackTrace();
            ...
            else if (type == BMessageEntity.Type.AUDIO || type == BMessageEntity.Type.VIDEO)
            {
                url = urls[0];
            }
...
//                if (DEBUG) Timber.d("dimensions[0]: %s, dimensions[1]: %s", dimensions[0], dimensions[1]);
            ...
            else if (resourcePath != null && VolleyUtils.getBitmapCache().contains(VolleyUtils.BitmapCache.getCacheKey(resourcePath)))
            {
    ...
    class ViewHolder{
        ...
        ChatBubbleAudioView audio;
        ChatBubbleVideoView video;
        TextView txtContent;
