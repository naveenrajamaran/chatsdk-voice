...
import com.braunster.chatsdk.object.SaveFileProgress;
...
import static com.braunster.chatsdk.dao.entities.BMessageEntity.Type.AUDIO;
...
import static com.braunster.chatsdk.dao.entities.BMessageEntity.Type.VIDEO;
...
public abstract class AbstractNetworkAdapter {
    ...
    public  Promise<BMessage, BError, BMessage> sendMessageWithLocation(final String filePath, final LatLng location, long threadId) {
        ...
        uploadImage(image, thumbnail)
                .progress(new ProgressCallback<SaveFileProgress>() {
                    @Override
                    public void onProgress(SaveFileProgress saveFileProgress) {
                        deferred.notify(message);
                    }
                })
    ...
    /**
     * Preparing a file message,
     * This is only the build part of the send from here the message will passed to "sendMessage" Method.
     * From there the message will be uploaded to the server if the upload fails the message will be deleted from the local db.
     * If the upload is successful we will update the message entity so the entityId given from the server will be saved.
     * When done or when an error occurred the calling method will be notified.
     *
     * @param filePath is a file that contains the image or audio or video. For now the file will be decoded to a Base64 image representation.
     * @param threadId the id of the thread that the message is sent to.
     */
    public Promise<BMessage, BError, BMessage> sendMessageWithFile(final String filePath, long threadId, int requestCode) {

        if(requestCode == ChatSDKChatHelper.SEND_IMAGE) return sendMessageWithImage(filePath, threadId);
        ...
        if(BDefines.Options.ShowFileAsLink) {
            message.setType(TEXT);
        } else {
            if(requestCode == ChatSDKChatHelper.SEND_IMAGE) message.setType(IMAGE);
            if(requestCode == ChatSDKChatHelper.SEND_AUDIO) message.setType(AUDIO);
            if(requestCode == ChatSDKChatHelper.SEND_VIDEO) message.setType(VIDEO);
        }
        ...
        if(message.getType() == AUDIO) {
            ...
            mmr.setDataSource(message.getResourcesPath());
            ...
            final String durationString = df.format(durationDouble);

            uploadAudio(FileUtils.getCompressed(filePath))
                    .progress(new ProgressCallback<SaveFileProgress>() {
                        @Override
                        public void onProgress(SaveFileProgress saveFileProgress) {
                            deferred.notify(message);
                    ...
                    .done(new DoneCallback<String>() {
                        ...
                        public void onDone(String url) {
                            if(BDefines.Options.ShowFileAsLink) {
                                ...
                                message.setText(url + BDefines.DIVIDER + durationString);
                                message.setDuration((int) Math.round(Double.valueOf(message.getDuration())));
                            }
        ...
        if(message.getType() == VIDEO) {
            String path = message.getResourcesPath();
            Bitmap thumbnail = ThumbnailUtils.createVideoThumbnail(path, MediaStore.Images.Thumbnails.MINI_KIND);
            message.setImageDimensions(ImageUtils.getDimensionAsString(thumbnail));
            Bitmap overlay = BitmapFactory.decodeResource(context.getResources(), R.drawable.play_button);
            thumbnail = ImageUtils.overlayBitmaps(thumbnail, overlay);

            VolleyUtils.getBitmapCache().put(
                    VolleyUtils.BitmapCache.getCacheKey(message.getResourcesPath()),
                    thumbnail);

            uploadVideo(FileUtils.getCompressed(filePath), thumbnail)
                    .progress(new ProgressCallback<SaveFileProgress>() {
                        @Override
                        public void onProgress(SaveFileProgress saveFileProgress) {
                            deferred.notify(message);
                    ...
                    .done(new DoneCallback<String[]>() {
                        ...
                        public void onDone(String[] url) {
                            if(BDefines.Options.ShowFileAsLink) {
                                message.setText(url[0]);
                            } else {
                                // VideoUrl, ThumbnailUrl, Dimensions
                                message.setText(url[0] + BDefines.DIVIDER + url[1] + BDefines.DIVIDER + message.getImageDimensions());
                            }
    ...
    private Deferred<BMessage, BError, BMessage> sendMessage(final BMessage message, final Deferred<BMessage, BError, BMessage> deferred){
        ...
        sendMessage(message).done(new DoneCallback<BMessage>() {
            @Override
            public void onDone(BMessage message) {
                if(DEBUG) Timber.v("message has been sent");
...
//                deferred.notify(message);
    ...
    public Promise<String[], BError, SaveFileProgress> uploadImage(final Bitmap image, final Bitmap thumbnail) {

        if(image == null || thumbnail == null) return rejectMultiple();

        final Deferred<String[], BError, SaveFileProgress> deferred = new DeferredObject<String[], BError, SaveFileProgress>();
    ...
    public Promise<String, BError, SaveFileProgress> uploadImageWithoutThumbnail(final Bitmap image) {
    ...
    public Promise<String, BError, SaveFileProgress> uploadAudio(final byte[] audio) {

        if(audio == null) return reject();

        final Deferred<String, BError, SaveFileProgress> deferred = new DeferredObject<String, BError, SaveFileProgress>();

        uploadHandler.uploadFile(audio, "audio.3gp", "audio/3gp")
                .progress(new ProgressCallback<SaveFileProgress>() {
                    @Override
                    public void onProgress(SaveFileProgress saveFileProgress) {
                        deferred.notify(saveFileProgress);
                    }
                })
                .done(new DoneCallback<String>() {
                    @Override
    ...
    public Promise<String[], BError, SaveFileProgress> uploadVideo(final byte[] video, final Bitmap thumbnail) {

        if(video == null || thumbnail == null) return rejectMultiple();

        final Deferred<String[], BError, SaveFileProgress> deferred = new DeferredObject<String[], BError, SaveFileProgress>();

        final String[] urls = new String[2];

        uploadHandler.uploadFile(video, "video.mp4", "video/mp4")
                .done(new DoneCallback<String>() {
                    @Override
                    public void onDone(String url) {
                        urls[0] = url;
    ...
    private static Promise<String, BError, SaveFileProgress> reject(){
        return new DeferredObject<String, BError, SaveFileProgress>().reject(new BError(BError.Code.NULL, "Image Is Null"));
    }

    private static Promise<String[], BError, SaveFileProgress> rejectMultiple(){
        return new DeferredObject<String[], BError, SaveFileProgress>().reject(new BError(BError.Code.NULL, "Image Is Null"));
    }
}
